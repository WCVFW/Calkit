...
import { UserPlusIcon, CheckCircleIcon, CurrencyRupeeIcon, ListBulletIcon, ClockIcon, FolderOpenIcon } from "@heroicons/react/24/outline";
...
const StatusBadge = ({ status }) => {
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-semibold ${colorClass}`}>
...
const leadData = [
    { id: 1, name: "Alice Smith", source: "Website", service: "GST Registration", status: "New", assigned: "Employee A" },
    { id: 2, name: "Bob Johnson", source: "Referral", service: "Trademark Filing", status: "In Progress", assigned: "Employee B" },
    { id: 3, name: "Charlie Brown", source: "Email Campaign", service: "Company Closure", status: "Dead", assigned: "Employee A" },
    { id: 4, name: "Diana Prince", source: "Website", service: "PF/ESI Registration", status: "New", assigned: "Employee C" },
];

const taskData = [
    { id: 101, title: "Follow up with Alice Smith", status: "Pending", dueDate: "2025-10-16", assignedTo: "Employee A" },
    { id: 102, title: "Draft Trademark application for Bob", status: "In Progress", dueDate: "2025-10-20", assignedTo: "Employee B" },
    { id: 103, title: "Prepare quarterly sales report", status: "Completed", dueDate: "2025-10-10", assignedTo: "Manager" },
];

const orderData = [
    { id: 1001, client: "Alice Smith", service: "GST Registration", value: 499, status: "Pending" },
    { id: 1002, client: "Acme Corp", service: "Annual Compliance", value: 14999, status: "Won" },
    { id: 1003, client: "Luna Biz", service: "Import Export Code", value: 999, status: "Lost" },
];

const stats = [
    { name: "New Leads", value: leadData.filter(l => l.status === 'New').length, icon: UserPlusIcon, color: "text-[#0074e0]", bgColor: "bg-white", border: "border-[#0074e0]" },
    { name: "Pending Tasks", value: taskData.filter(t => t.status === 'Pending').length, icon: ClockIcon, color: "text-[#FF453A]", bgColor: "bg-white", border: "border-[#FF453A]" },
    { name: "Total Orders", value: orderData.length, icon: FolderOpenIcon, color: "text-[#00A859]", bgColor: "bg-white", border: "border-[#00A859]" },
    { name: "Revenue Won (Sample)", value: "₹15,498", icon: CurrencyRupeeIcon, color: "text-[#00A859]", bgColor: "bg-white", border: "border-[#00A859]" },
];
...
const LeadsTable = () => (
    <div className="bg-white rounded-xl shadow p-6 border-t-4 border-[#0074e0]">
        <div className="flex justify-between items-center mb-4">
            ...
            <Link to="/crm/leads" className="text-sm font-medium text-[#0074e0] hover:text-[#005bb5]">
                View All Leads &gt;
            </Link>
        </div>
        ...
        <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                    {leadData.map((lead) => (
                        <tr key={lead.id} className="hover:bg-gray-50 transition duration-100">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <Link to={`/crm/leads/${lead.id}`} className="text-[#0074e0] hover:text-[#005bb5]">Open</Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
...
const TasksTable = () => (
    <div className="bg-white rounded-xl shadow p-6 border-t-4 border-[#FFA900]">
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-gray-800">Employee Task Assignments</h2>
            <Link to="/crm/tasks" className="text-sm font-medium text-[#FFA900] hover:text-[#cc8600]">
                View All Tasks &gt;
            </Link>
        </div>
        ...
        <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                    {taskData.map((task) => (
                        <tr key={task.id} className="hover:bg-gray-50 transition duration-100">
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                {task.title}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <Link to={`/crm/tasks/${task.id}`} className="text-[#FFA900] hover:text-[#cc8600]">Edit</Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
...
const OrdersTable = () => (
    <div className="bg-white rounded-xl shadow p-6 border-t-4 border-[#00A859]">
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-gray-800">Order/Sales Management</h2>
            <Link to="/crm/orders" className="text-sm font-medium text-[#00A859] hover:text-[#008945]">
                View Full Pipeline &gt;
            </Link>
        </div>
