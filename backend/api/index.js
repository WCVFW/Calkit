...
app.post("/api/auth/signup", async (req, res) => {
  ...
  if (!email || !password || !name || !phone)
    return res
      .status(400)
      .json({ error: "Name, email, phone, password required" });
  try {
...
app.post("/api/auth/verify-email", async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      "SELECT id, expires_at FROM email_otps WHERE email=? AND code=? ORDER BY id DESC LIMIT 1",
      [email, code],
    );
    if (rows.length === 0)
...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      "SELECT id, name, email, phone, password_hash, is_verified FROM users WHERE email=?",
      [email],
    );
    if (rows.length === 0)
...
app.post("/api/auth/verify-phone", async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      "SELECT id, expires_at FROM phone_otps WHERE phone=? AND code=? ORDER BY id DESC LIMIT 1",
      [phone, code],
    );
    if (rows.length === 0)
      return res.status(400).json({ error: "Invalid code" });
    ...
    const [u] = await pool.query(
      "SELECT id, name, email, phone, is_verified FROM users WHERE phone=?",
      [phone],
    );
    let user;
    if (u.length === 0) {
      ...
      const result = await pool.query(
        ...
        [placeholder, `${placeholder}@example.com`, phone, hash],
      );
...
app.get("/api/leads", authMiddleware, async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT id, name, service, status, created_at FROM leads WHERE owner_user_id=? ORDER BY id DESC",
      [req.userId],
    );
    res.json(rows);
  } catch (e) {
...
app.post("/api/leads", authMiddleware, async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      "INSERT INTO leads (owner_user_id, name, service, status) VALUES (?,?,?,?)",
      [req.userId, name, service || null, status],
    );
...
app.put("/api/leads/:id", authMiddleware, async (req, res) => {
  ...
  try {
    await pool.query(
      "UPDATE leads SET name=COALESCE(?, name), service=COALESCE(?, service), status=COALESCE(?, status) WHERE id=? AND owner_user_id=?",
      [name || null, service || null, status || null, id, req.userId],
    );
...
app.delete("/api/leads/:id", authMiddleware, async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query("DELETE FROM leads WHERE id=? AND owner_user_id=?", [
      id,
      req.userId,
    ]);
